...
def _make_gold_posts(search_requests: list[dict]) -> list[dict]:
    ...
    tweets = [
        ...
        (
            "positive",
            0.88,
            "Building ETL with Apache Spark is incredibly rewarding #dataengineering",
        ),
        (
            "negative",
            0.75,
            "Data quality issues are destroying our pipeline reliability #dataengineering",
        ),
        (
            "neutral",
            0.60,
            "Interesting discussion on batch vs streaming architectures #dataengineering",
        ),
        (
            "positive",
            0.85,
            "Excited about the future of real-time data processing #dataengineering",
        ),
    ]
    ...
    fb_posts = [
        (
            "positive",
            0.89,
            "Just trained a model with 98% accuracy! ML is amazing #machinelearning",
        ),
        ...
        (
            "neutral",
            0.65,
            "Comparing TensorFlow vs PyTorch for production workloads #machinelearning",
        ),
    ]
...
def streamlit_process(_seed_db: Path):  # type: ignore[no-untyped-def]
    ...
    while time.time() < deadline:
        try:
            resp = urllib.request.urlopen(HEALTH_URL, timeout=2)  # noqa: S310
            if resp.status == 200:
                break
        except Exception:
            time.sleep(0.5)
...
def navigate_to(page, screen_name: str) -> None:
    ...
    option = page.wait_for_selector(f'li[role="option"]:has-text("{screen_name}")', timeout=5000)
    option.click()
