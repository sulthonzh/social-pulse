...
class EnrichedPostRepository(Protocol):
    ...
    def get_by_search_paginated(
        self, search_request_id: str, limit: int = 1000, offset: int = 0
    ) -> list[EnrichedPost]: ...
    def get_enriched_since_paginated(
        self, search_request_id: str, since: datetime, limit: int = 1000, offset: int = 0
    ) -> list[EnrichedPost]: ...
...
class GoldBuildTrackingRepository(Protocol):
    """Repository for tracking incremental gold build progress per search request."""
